...
class ComponentProvider extends ChangeNotifier {
  ...
  void searchOnUserComponents(searchString) {
    ...
    List<String> items = copyOfUserComponents.values
        ...
        .where(
            (itemName) => itemName != null) // Optional: Ensure no null values
        .toList();
  ...
  void searchOnAllComponents(searchString) {
    ...
    List<String> items =
        ListOfitems.map((item) => item['item name'] as String).toList();
  ...
  void printmessage(String message) {
    Fluttertoast.showToast(
      ...
      backgroundColor:
          const Color.fromARGB(255, 48, 48, 48), // Background color
        ...
        if (user != null) {
          ...
          if (await isBorrowed(componentCode) == true) {
            ...
            await firestore.collection('items').doc(itemId).update({
              ...
              'inLabComponents':
                  FieldValue.increment(-1), // Decrease the count by 1
            });
...
Future<void> setLabled(String componentCode) async {
  ...
  Future<void> returnComponent(String componentCode) async {
    ...
    try {
      if (user != null) {
        ...
        await firestore.collection('items').doc(itemId).update({
          'instances.$componentCode.borrowed_by':
              "", // Clear the borrowed_by field
          'inLabComponents': FieldValue.increment(
              1), // Increment the inLabComponents count by 1
        });
  ...
  List<String> getAllComponentCodesForItemId(String itemId) {
    ...
    List<String> componentCodes = (item["instances"] as Map<String, dynamic>)
        .entries
        .where((entry) =>
            entry.value['borrowed_by'] ==
            '') // Filter where 'borrowed_by' is ''
    ...
    try {
      if (user != null) {
        ...
        DocumentReference docRef =
            _firestore.collection('actions').doc(componentCode);
